...
class Simulacion:
    ...
    def load_data(self, calles):
        ...
        for cuadrante in calles:
            self.camiones_chicos.append(Camion_chico(cuadrante, calles[cuadrante]))
    ...
    def dia_simulacion(self, dia_sim):
        tiempo_simulacion = 0
        basura_depositada_camiones_chicos = 0
        self.grid.generar_basura()
        print(self.grid)
        #self.grid.mapa()
        for camion in self.camiones_chicos:
            camion.inicio_dia(dia_sim)
        ...
        while tiempo_simulacion < 11 * 3600:
            ...
            if isinstance(camion_actual, Camion_chico): #Si el camion que realiza el siguiente evento es chico:
                ...
                if camion_actual.status == "RECOLECTANDO":
                ...
                elif camion_actual.status == "YENDO AL DEPOSITO":
                    camion_actual.tiempo_en_recolectar(tiempo_simulacion)
                    ...
                    elif len(self.camion_grande.cola_vaciado) >= 3 and self.camion_grande.status == "CENTRO DE ACOPIO" or self.camion_grande.status =="VACIADO":
                        self.camion_grande.cola_vaciado.pop(-1)
                        camion_actual.tiempo_traslado_deposito(tiempo_simulacion)
                        basura_depositada_camiones_chicos += camion_actual.basura_actual
                        camion_actual.basura_actual = 0
                ...
                if camion_actual == None:
                    break
                print(" --------------------- asdasd --- ")
                print(self.camion_grande.tiempo_siguiente_evento)
        ...
        for camion in self.camiones_chicos: #hay que cambiar esto.
            ...
            #print("basura actual {} tiempo evento {} basura total recogida {}".format(camion.basura_actual,camion.tiempo_siguiente_evento, camion.basura_total_recogida))
    ...
    def mes_simulacion(self):
        dia_actual = 0
        while dia_actual < 28:
            self.dia_simulacion(dia_actual)
...
calles_en_cuadrantes_final = {}
for x in range (1,18):
    calles_en_cuadrantes = [[],[],[]]
    for data in calles:
        calle2 = Calle(data, calles[data]["comienza"][1], calles[data]["comienza"][0], calles[data]["termina"][1], calles[data]["termina"][0], calles[data]["cuadrante"], calles[data]["subcuadrante"])
        if calles[data]["cuadrante"] == x and calles[data]["subcuadrante"] == 1:
            calles_en_cuadrantes[0].append(calle2)
            grid.add_calle(calle2)
        elif calles[data]["cuadrante"] == x and calles[data]["subcuadrante"] == 2:
            calles_en_cuadrantes[1].append(calle2)
            grid.add_calle(calle2)
        elif calles[data]["cuadrante"] == x and calles[data]["subcuadrante"] == 3:
            calles_en_cuadrantes[2].append(calle2)
            grid.add_calle(calle2)

    calles_en_cuadrantes_final[x] = calles_en_cuadrantes


simulacion.load_data(calles_en_cuadrantes_final)

for camion in simulacion.camiones_chicos:
    camion.definir_orden_recoleccion()
simulacion.dia_simulacion(1)
