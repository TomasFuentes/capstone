...
class Camion_chico:
    ...
    def __init__(self,cuadrante, calles_cuadrante_a_recolectar):
        ...
        self.tiempo_al_deposito = 30000 / self.velocidad_desplazamiento_sinrecolectar
        ...
        self.calles_por_recolectar_dia1 = len(calles_cuadrante_a_recolectar[0])
        self.calles_por_recolectar_dia2 = len(calles_cuadrante_a_recolectar[1])
        self.calles_por_recolectar_dia3 = len(calles_cuadrante_a_recolectar[2])
        self.calles_a_recolectar = calles_cuadrante_a_recolectar
        self.ruteo_recoleccion = [[], [], []]
        self.ultima_calle = None

    def __str__(self):
        return "El camión chico '{}' debe recolectar {} calles en el cuadrente {}".format(self.id, self.calles_a_recolectar, self.cuadrante)

    def inicio_dia(self,dia_simulacion): 
        if dia_simulacion % 3 == 0:
            self.calles_a_recolectar = self.ruteo_recoleccion[2]
        elif dia_simulacion % 3 == 1:
            self.calles_a_recolectar = self.ruteo_recoleccion[1]
        else:
            self.calles_a_recolectar = self.ruteo_recoleccion[0]
    ...
    def recoleccion(self):
        ...
        self.ultima_calle = calle
        if self.basura_actual >= self.capacidad_maxima:
            calle.basura_actual = self.basura_actual - self.capacidad_maxima
    ...
    def definir_orden_recoleccion(self):  ## debemos crear la función de ruteo para generar una LISTA ORDENADA, que defina el movimiento a
        for lista in range(0, 3):
            ...
            subiendo = True
            orden = []
            for calle in self.calles_a_recolectar[lista]:
                    ...
                    for calle in self.calles_a_recolectar[lista]:
                        if subiendo:
                            if calle.inicio_x == xactual and calle.inicio_y == yactual and calle.termino_x == xactual \
                                ...
                                self.calles_a_recolectar[lista].remove(calle)
                                if yactual + 1 >= ymax:
                                    calle.borde = True
                        else:
                ...
                while (True):
                    ...
                    if devolviendo == True:
                        ...
                        if xactual >= xmax:
                            devolviendo = True
                            break
            self.ruteo_recoleccion[lista] = orden
            print(orden)
            print("--------------------")
    ...
    def tiempo_traslado_acopio(self, tiempo_simulacion):
        ...
        self.tiempo_siguiente_evento = tiempo + self.tiempo_desde_origen((self.ultima_calle.termino_x, self.ultima_calle.termino_y))
        return self.tiempo_siguiente_evento
    
    def tiempo_traslado_cuadrante(self, tiempo_simulacion):
        tiempo = tiempo_simulacion
        self.status = "YENDO A CUADRANTE"
        if len(self.calles_a_recolectar) > 0:
    ...
    def tiempo_traslado_deposito(self, tiempo_simulacion):
        tiempo = tiempo_simulacion
        self.status = "YENDO AL DEPOSITO"
        self.tiempo_siguiente_evento = tiempo + self.tiempo_al_deposito * 2
        self.tiempo_siguiente_evento += 20 * 60
        return self.tiempo_siguiente_evento
...
class Camion_grande:
    ...
    def descarga_camion_chico(self, tiempo_simulacion):
        ...
        if len(self.cola_vaciado) > 0:
            ...
            if self.status == "CENTRO DE ACOPIO":
                self.tiempo_siguiente_evento = infinito
